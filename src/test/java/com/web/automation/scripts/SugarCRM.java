...
public class SugarCRM extends TestEngineWeb {
	...
	public void contactValidation() throws Throwable {
		try {
			...
			// ------------------------------------------------------------------------------------			
			// **************DeleteContacts********************************************************
				isDeleteExistingSuccessfull=SugarPage.deleteContact(testData);
				if(isDeleteSuccessfull){
				testCaseFailureReason = "Deletion of Existing contact card failed";				
				extenLogs.fail("SugarCRM ", testCaseFailureReason + "Failed Reason : "
			...
			// **************VerifyContacts********************************************************
			// ------------------------------------------------------------------------------------
			IsVerificationOfContactSuccessfull=SugarPage.verifyContactDetails(testData);
			if(!IsVerificationOfContactSuccessfull){
				testCaseFailureReason = "Data Validation of the contact card failed";				
				extenLogs.fail("SugarCRM ", testCaseFailureReason + "Failed Reason : "
						+ "Data does not match with the"+ "entered data");
			...
			// ------------------------------------------------------------------------------------			
			// **************DeleteContacts********************************************************
			isDeleteSuccessfull=SugarPage.deleteContact(testData);
			if(isDeleteSuccessfull){
				testCaseFailureReason = "Deletion of contact card failed";				
				extenLogs.fail("SugarCRM ", testCaseFailureReason + "Failed Reason : "
						+ "Contact was not found");						
				testCaseStatus = false;
				throw new Exception();							
