...
public class ExtentManager extends TestEngineWeb {
	...
	public static String timestamp = new SimpleDateFormat("yyyy_MM_dd_hh_mm_sss_a").format(new Date());
	...
	private synchronized static ExtentReports getExtentReport() {
		strFolderPath=System.getProperty("user.dir").replace("\\", "/")+"/logs/Reports/local/ExtentReports_"+timestamp;
		DetailedLogs.CreateFolder(strFolderPath);	
		if (reporter == null) {
			if(System.getenv("jenkins_home")==null){
				CommonVariables.extentReportPath.set(System.getProperty("user.dir").replace("\\", "/")+"/logs/Reports/local/"+"ExtentReports_"+timestamp +"/ExtentReports_"+timestamp+".html");
			}else{
			...
			strScreenPath=strFolderPath+"/Screenshot";
			DetailedLogs.CreateFolder(strScreenPath);	
		}
		return reporter;
	}
